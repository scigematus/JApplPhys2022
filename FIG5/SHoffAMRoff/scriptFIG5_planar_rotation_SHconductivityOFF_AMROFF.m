...
amr = 1.00;%AMR ratio
%
s_tt = 0*s*0.005;%sigma_she^tt
s_lt = 0*s*0.0075;
s_tl = 0*s*0.0075;
s_itt = 0*s*0.005;%sigma_ishe^tt
s_ilt = 0*s*0.0075;
s_itl = 0*s*0.0075;
%
s_a = s_tt*b;
...
A = [0;-s*lam_l^-2;-s*lam_l^-2;-s*lam_l^-2];
F = [0;0;0;0];
G0 = [0;0;0;0];
%% 
...
for i = 1:N
    ...
    rotate(gm,-theta(i),[0 0 0],[0 0 lz]);
    model = createpde(4);
    model.Geometry = gm;
    ...
    yq = linspace(0.49*lx*sind(theta(i)),-0.49*lx*sind(theta(i)),2*fine+1);
    zq = linspace(lz/2,lz/2,2*fine+1);
    uintrp = interpolateSolution(result,xq,yq,zq,[1,2,3,4]);
    position = cat(1, position, cat(2,-sqrt(yq(1:fine).^2 + xq(1:fine).^2), sqrt(xq(fine+1:2*fine+1).^2 + yq(fine+1:2*fine+1).^2)));
